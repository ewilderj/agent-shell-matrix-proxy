...
#+begin_example
┌─────────────────────┐         ┌──────────────────────┐         ┌─────────────────┐
│      Emacs          │         │  Matrix Proxy Bot    │         │ Matrix Homeserver│
│  ┌───────────────┐  │  HTTP   │  (Python/matrix-nio) │ Matrix  │                 │
│  │  agent-shell  │◄─┼────────►│                      │◄───────►│  ┌───────────┐  │
│  └───────┬───────┘  │webhooks │  • Session mgmt      │protocol │  │  Element   │  │
│          │          │         │  • Message relay      │         │  │  (mobile)  │  │
│  ┌───────┴───────┐  │         │  • Rate limiting     │         │  └───────────┘  │
│  │   webhook     │  │         │  • !commands          │         │  ┌───────────┐  │
│  │   server :9999│◄─┼─────────┤  • SQLite sessions   │         │  │  Element   │  │
│  └───────────────┘  │callback │                      │         │  │   (web)    │  │
└─────────────────────┘         └──────────────────────┘         │  └───────────┘  │
                                                                 └─────────────────┘

Flow:
  Handoff: Emacs ──POST /handoff──► Bot ──creates room──► Matrix
  Relay:   Emacs ◄──── webhooks ────► Bot ◄──── messages ────► Matrix
  Return:  Matrix ──!return──► Bot ──POST /webhook──► Emacs
#+end_example
