...
#+begin_example
  Emacs                        Matrix Proxy Bot              Matrix
  +-------------------+        +--------------------+        +----------------+
  | agent-shell       |--POST /handoff------------->|        |                |
  |                   |--POST /webhook/message----->|        |                |
  |                   |        |                    |<======>| Homeserver     |
  | webhook :9999     |<--POST /webhook callback----|        |                |
  +-------------------+        +--------------------+        |  Element       |
                               | Session mgmt       |        |  (mobile/web)  |
                               | Message relay      |        +----------------+
                               | Rate limiting      |
                               | !commands          |
                               +--------------------+
...
- [[https://github.com/ElleNajt/agent-shell-to-go][agent-shell-to-go]] — Similar concept using Slack instead of Matrix. Pure elisp, no external bot process.
